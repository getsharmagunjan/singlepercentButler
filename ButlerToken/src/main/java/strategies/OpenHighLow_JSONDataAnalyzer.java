...
import ButlerToken.OrderProcessor;

public class OpenHighLow_JSONDataAnalyzer 
{
	...
	OpenHighLow_JSONDataAnalyzer()
	{
		...
		this.sheet=new OpenHighLow_GoogleSpreadsheetHandler();
	...
	private void analyzeJSONData()
	{
		...
		if(((double)(new File(this.nifty100_new_json_data_file_path).length()/1024) > 5.00d))
		{
			...
			this.getNewNifty100Data(this.nifty100_file_path, 0, 101);
		}
		else
		{
			this.nifty100_file_path=this.nifty100_old_json_data_file_path;
			this.getOldNifty100Data(this.nifty100_file_path, 0, 101);
		}
	...
	{
		OpenHighLow_JSONDataAnalyzer obj=new OpenHighLow_JSONDataAnalyzer(); 
